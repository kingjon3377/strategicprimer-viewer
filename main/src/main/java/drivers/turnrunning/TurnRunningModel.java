...
public class TurnRunningModel extends ExplorationModel implements ITurnRunningModel {
	...
	public <T extends HasPopulation<? extends TileFixture>&TileFixture>
		...
		if (reduction > 0) {
			...
			for (IMutableMapNG map : getRestrictedAllMaps()) {
				T matching = (T) map.getFixtures(location).stream()
				...
				if (matching != null) {
					if (all) {
						...
						int remaining = matching.getPopulation() - reduction;
						if (remaining > 0) {
							T addend = (T) matching.reduced(remaining);
	...
	public boolean replaceSkillInJob(IWorker worker, String jobName, ISkill delenda, ISkill replacement) {
		...
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			IMutableWorker matching = StreamSupport.stream(map.getLocations().spliterator(), true)
				...
				.filter(w -> w.getId() == worker.getId()).findAny().orElse(null);
			if (matching != null) {
				IMutableJob matchingJob = StreamSupport.stream(matching.spliterator(), true)
					.filter(IMutableJob.class::isInstance).map(IMutableJob.class::cast)
					.filter(j -> jobName.equals(j.getName())).findAny().orElse(null);
				if (matchingJob != null) {
					ISkill matchingSkill = StreamSupport.stream(matchingJob.spliterator(), true)
	...
	public boolean reduceResourceBy(IResourcePile resource, BigDecimal amount, Player owner) {
		boolean any = false;
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			for (FixtureIterable<?> container : StreamSupport.stream(map.getLocations().spliterator(), true)
					.flatMap(l -> map.getFixtures(l).stream()).flatMap(TurnRunningModel::partiallyFlattenFortresses)
				...
				for (IMutableResourcePile item : StreamSupport.stream(container.spliterator(), false)
					...
					if (resource.isSubset(item, x -> {})) { // TODO: is that the right way around?
						BigDecimal qty = decimalize(item.getQuantity().getNumber());
	...
	public boolean setUnitOrders(IUnit unit, int turn, String results) {
		boolean any = false;
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			IMutableUnit matching = StreamSupport.stream(map.getLocations().spliterator(), true)
				.flatMap(l -> map.getFixtures(l).stream()).flatMap(TurnRunningModel::unflattenNonFortresses)
...
//				.filter(u -> u.getKind().equals(container.getKind()))
	...
	public boolean addResource(IFortress container, int id, String kind, String contents, Quantity quantity) {
		...
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			...
			IMutableFortress matching = StreamSupport.stream(map.getLocations().spliterator(), true)
...
//				.filter(u -> u.getKind().equals(container.getKind()))
	...
	public boolean addAnimal(IUnit container, String kind, String status, int id, int population, int born) {
		...
		Animal animal = new AnimalImpl(kind, false, status, id, born, population);
		boolean any = false;
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			IMutableUnit matching = StreamSupport.stream(map.getLocations().spliterator(), true)
				.flatMap(l -> map.getFixtures(l).stream()).flatMap(TurnRunningModel::unflattenNonFortresses)
	...
	public boolean transferResource(IResourcePile from, IUnit to, BigDecimal quantity, IntSupplier idFactory) {
		...
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			for (FixtureIterable<?> container : StreamSupport.stream(map.getLocations().spliterator(), true)
				...
				IMutableUnit destination = StreamSupport.stream(map.getLocations().spliterator(), true)
					.flatMap(l -> map.getFixtures(l).stream()).flatMap(TurnRunningModel::partiallyFlattenFortresses)
					.filter(IMutableUnit.class::isInstance).map(IMutableUnit.class::cast)
				...
				if (matching != null && destination != null) {
					...
					if (quantity.doubleValue() >= matching.getQuantity().getNumber().doubleValue()) {
						...
						IMutableResourcePile split = new ResourcePileImpl(id.getAsInt(),
		...
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			for (FixtureIterable<?> container : StreamSupport.stream(map.getLocations().spliterator(), true)
				...
				IMutableResourcePile matching = StreamSupport.stream(container.spliterator(), false)
					...
					.filter(r -> r.getId() == from.getId()).findAny().orElse(null);
				// TODO: Match destination by owner and kind, not just name and ID?
				IMutableFortress destination = StreamSupport.stream(map.getLocations().spliterator(), true)
					.flatMap(l -> map.getFixtures(l).stream())
					.filter(IMutableFortress.class::isInstance).map(IMutableFortress.class::cast)
		...
		for (IMutableMapNG map : getRestrictedAllMaps()) {
			Supplier<Stream<IMutableFortress>> supp =
				() -> StreamSupport.stream(map.getLocations().spliterator(), true)
					...
					.filter(IMutableFortress.class::isInstance).map(IMutableFortress.class::cast)
					.filter(f -> f.getOwner().equals(owner));
			IMutableFortress result = supp.get().filter(f -> fortName.equals(f.getName())).findAny()
