...
public class DrawHelperComparator implements UtilityDriver {
		...
		for (int rep = 0; rep < reps; rep++) {
			...
			for (final Point point : map.getLocations()) {
				helper.drawTile(image.createGraphics(), map, point,
						new Coordinate(point.row() * tileSize, point.column() * tileSize),
						dimensions);
			}
		}
	...
	private static long second(final TileDrawHelper helper, final ILegacyMap map, final int reps, final int tileSize) {
		...
		for (int rep = 0; rep < reps; rep++) {
			...
			final Coordinate dimensions = new Coordinate(tileSize, tileSize);
			for (final Point point : map.getLocations()) {
				helper.drawTile(pen, map, point, new Coordinate(point.row() * tileSize,
	 ...
	 */
	private static long thirdOne(final TileDrawHelper helper, final ILegacyMap map, final int reps,
								 final int tileSize) {
		final MapDimensions mapDimensions = map.getDimensions();
		final BufferedImage image = new BufferedImage(tileSize * mapDimensions.columns(),
				tileSize * mapDimensions.rows(), BufferedImage.TYPE_INT_RGB);
	...
	private static long thirdTwo(final TileDrawHelper helper, final ILegacyMap map, final int reps,
								 final int tileSize) {
		final MapDimensions mapDimensions = map.getDimensions();
		final BufferedImage image = new BufferedImage(tileSize * mapDimensions.columns(),
				tileSize * mapDimensions.rows(), BufferedImage.TYPE_INT_RGB);
	...
	private static final List<Pair<String, TestInterface>> TESTS =
			List.of(Pair.with("1. Basic Drawing", DrawHelperComparator::first),
	...
	private long printStats(final String prefix, final long total, final int reps) {
		cli.println("%s\t%d, average of %.1f ms.".formatted(prefix, total, total / (double) reps));
		return total;
	}
