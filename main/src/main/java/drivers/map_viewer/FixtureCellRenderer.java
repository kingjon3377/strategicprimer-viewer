...
/* package */ final class FixtureCellRenderer implements ListCellRenderer<TileFixture> {
	...
	public Component getListCellRendererComponent(final JList<? extends TileFixture> list,
												  final TileFixture val, final int index, final boolean isSelected,
												  final boolean cellHasFocus) {
		final JLabel component = (JLabel) DEFAULT_RENDERER.getListCellRendererComponent(list,
				val, index, isSelected, cellHasFocus);
		component.setText("<html><p>%s</p></html>".formatted(val.getShortDescription()));
		if (val instanceof final HasImage hi) {
