...
shared class FortressTabularReportGenerator(Player player, Point? hq,
    ...
    shared actual [{[String(), Anything(String)?]+}+] produce(
        ...
        {[String(), Anything(String)?]+} retval = [[distanceString(loc, hq, dimensions), null],
            locationEntry(loc), [() => ownerString(player, item.owner), null], // TODO: setter for owner?
            [() => item.name, (String str) => item.name = str]];
