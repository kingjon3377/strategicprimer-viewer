...
shared class DiggableTabularReportGenerator(Point? hq, MapDimensions dimensions)
    ...
    shared actual [{[String(), Anything(String)?]+}+] produce(
        ...
        case (is Ground) {
            classGetter = () => "ground";
            classSetter = null;
            statusGetter = () => (item.exposed) then "exposed" else "not exposed";
        ...
        case (is MineralVein) {
            ...
            log.error("Unhandled diggable type");
            return [];
        }
        fixtures.remove(key);
        return [[[() => distanceString(loc, hq, dimensions), null], locationEntry(loc),
            [classGetter, classSetter], [() => item.kind, (String str) => nothing],
            [statusGetter, statusSetter]]];
    }
