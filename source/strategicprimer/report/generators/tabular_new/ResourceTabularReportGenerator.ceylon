...
shared class ResourceTabularReportGenerator(Point? hq, MapDimensions dimensions)
    ...
    shared actual [{[String(), Anything(String)?]+}+] produce(
        ...
        String() quantityGetter;
        Anything(String)? quantitySetter;
        String() specificsGetter;
        Anything(String)? specificsSetter;
        switch (item)
        case (is ResourcePile) {
            kindGetter = () => item.kind;
            kindSetter = (String str) => item.kind = str;
            quantityGetter = () => item.quantity.string;
            quantitySetter = (String str) => nothing;
            specificsGetter = () => item.contents;
            specificsSetter = (String str) => item.contents = str;
        }
        case (is Implement) {
            kindGetter = () => "equipment";
            kindSetter = null;
            quantityGetter = () => item.count.string;
            quantitySetter = (String str) => nothing;
            specificsGetter = () => item.kind;
            specificsSetter = (String str) => item.kind = str;
        }
        case (is ResourcePile) {
            kindGetter = () => item.kind;
            kindSetter = (String str) => item.kind = str;
            quantityGetter = invalidGetter;
            quantitySetter = null;
            specificGetter = () => item.contents;
            specificsSetter = (String str) => item.contents = str;
        }
        fixtures.remove(key);
        return [[[distanceString(loc, hq, dimensions), null], locationEntry(loc),
            [kindGetter, kindSetter], [quantityGetter, quantitySetter], [specificsGetter, specificsSetter]]];
    }
    ...
    // FIXME: Port this to the editable-row framework
