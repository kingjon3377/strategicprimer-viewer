...
object dbFieldWriter satisfies DatabaseWriter<Meadow, Point> {
	shared actual void write(Sql db, Meadow obj, Point context) {
		db.Statement("""CREATE TABLE IF NOT EXISTS fields (
			                row INTEGER NOT NULL,
			                column INTEGER NOT NULL,
			                id INTEGER NOT NULL,
			                type VARCHAR(6) NOT NULL CHECK (type IN ('field', 'meadow')),
			                kind VARCHAR(64) NOT NULL,
			                cultivated BOOLEAN NOT NULL,
			                status VARCHAR(7) NOT NULL CHECK (status IN ('fallow', 'seeding', 'growing', 'bearing')),
