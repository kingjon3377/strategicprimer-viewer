...
public class SPMap implements IMutableMap {
	...
	public final void replacePlayer(final Player toRemove, final Player toAdd) {
		...
		} else if (toRemove.playerId() != toAdd.playerId() && !players.getPlayer(toAdd.playerId()).getName().isEmpty()) {
			// TODO: Make some way to either get the player in the collection, *or nothing if none*, or else test if a player exists in the collection
