...
public abstract class AbstractChildNode<T> extends AbstractXMLNode {
	...
	public final void addProperty(final String property, final String value,
		...
		if ("line".equals(property)) {
			...
			warner.warn(new UnsupportedPropertyException(toString(), property,
					getLine()));
		}
	}
	 ...
	 */
	protected final String getPropertyWithDefault(final String property,
	 ...
	 *        one from.
	 * @param warner the Waring instance to use if necessary.
	 */
	protected void registerOrCreateID(final String tag,
			final IDFactory idFactory, final Warning warner) {
		if (hasProperty("id")) {
	 ...
	 * @param emptyOK whether an empty value is OK (if not, they count as
	 *        missing)
	...
	protected void demandProperty(final String tag, final String property,
		...
		if (!hasProperty(property)
				|| ((!emptyOK) && getProperty(property).isEmpty())) {
			final SPFormatException except = new MissingParameterException(tag,
					property, getLine());
			if (warning) {
				warner.warn(except);
			} else {
	...
	protected void handleDeprecatedProperty(final String tag,
		...
		if (!hasProperty(preferred)
				|| (!emptyValid && getProperty(preferred).isEmpty())) {
			if (hasProperty(deprecated)
					&& (emptyValid || !getProperty(deprecated).isEmpty())) {
				warner.warn(new DeprecatedPropertyException(tag, deprecated,
						preferred, getLine()));
				addProperty(preferred, getProperty(deprecated), warner);
			} else {
				final SPFormatException except = new MissingParameterException(
						tag, preferred, getLine());
				if (required) {
					throw except;
				} else {
					warner.warn(except);
