...
public class TableDebugger {
	...
	public void debugTables(final PrintStream out) throws MissingTableException {
		...
		debugTable("", "", mainTable, "main", out, new HashSet<EncounterTable>());
	}
	/**
	...
	private void debugTable(final String before, final String after,
			final EncounterTable table, final String tableName, final PrintStream out, final Set<EncounterTable> set)
			throws MissingTableException {
		if (set.contains(table)) {
			out.println("table " + tableName + " is already on the stack, skipping ...");
			out.println("The cause was: " + before + "#" + tableName + "#" + after);
			return;
		} else {
			set.add(table);
		}
		for (final String value : table.allEvents()) {
			if (value.contains("#")) {
				...
				debugTable(before + parsed[0], parsed[2] + after, runner.getTable(parsed[1]), parsed[1], out, set);
			} else {
				out.print(before);
				out.print(value);
				out.println(after);
