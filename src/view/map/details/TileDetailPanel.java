...
public class TileDetailPanel extends JPanel implements PropertyChangeListener {
	...
	/**
	 * Set the coordinate label appropriate based on the new selection.
	 * @param point the location we now represent.
	 */
	public void updateText(final Point point) {
		coordLabel.setText("<html>Coordinates: "
				+ point.toString() + "</html>");
	}
	/**
	 * Set the terrain label appropriately based on a new tile.
	 *
	...
	public void propertyChange(final PropertyChangeEvent evt) {
		if ("tile".equals(evt.getPropertyName())) {
			...
			repaint();
		} else if ("point".equals(evt.getPropertyName())) {
			updateText((Point) evt.getNewValue());
			repaint();
		}
	}
}
