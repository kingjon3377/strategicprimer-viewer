...
public final class TestMoreFixtureSerialization extends
	...
	public void testGroveSerialization() throws XMLStreamException,
		...
		assertSerialization("First test of Grove serialization, reflection",
				new Grove(true, true, "firstGrove", 1, FAKE_FILENAME), Grove.class);
		assertSerialization("Second test of Grove serialization, reflection",
				new Grove(true, false, "secondGrove", 2, FAKE_FILENAME), Grove.class);
		assertSerialization("Third test of Grove serialization, reflection",
				new Grove(false, true, "thirdGrove", 3, FAKE_FILENAME), Grove.class);
	...
	public void testMineSerialization() throws XMLStreamException,
		...
		assertSerialization("First test of Mine serialization, reflection",
				new Mine("one", TownStatus.Active, 1, FAKE_FILENAME), Mine.class);
		assertSerialization("Second test of Mine serialization, reflection",
				new Mine("two", TownStatus.Abandoned, 2, FAKE_FILENAME), Mine.class);
		assertSerialization("Third test of Mine serialization, reflection",
				new Mine("three", TownStatus.Burned, 3, FAKE_FILENAME), Mine.class);
		...
		assertDeprecatedDeserialization("Deprecated Mine idiom", four,
				createSerializedForm(four, false).replace(KIND_PROPERTY, oldKindProperty),
	...
	public void testShrubSerialization() throws XMLStreamException,
		...
		assertDeprecatedDeserialization(
				...
				createSerializedForm(two, false).replace(KIND_PROPERTY, oldKindProperty),
	...
	public void testVillageSerialization() throws XMLStreamException,
		...
		for (TownStatus status : TownStatus.values()) {
			final Village one = new Village(status, "villageOne", 1, FAKE_FILENAME); // NOPMD
			assertSerialization("First Village serialization test, reflection, "
					+ status, one, Village.class);
			final Village two = new Village(status, "villageTwo", 2, FAKE_FILENAME); // NOPMD
			assertSerialization("Second Village serialization test, reflection, "
					+ status, two, Village.class);
		}
		final Village three = new Village(TownStatus.Abandoned, "", 3, FAKE_FILENAME);
		...
		assertMissingPropertyDeserialization(
				"Serialization of village with no or empty name does The Right Thing",
				three, createSerializedForm(three, false), Village.class, NAME_PROPERTY);
	...
	public void testUnitWarnings() throws XMLStreamException, SPFormatException, IOException { // NOPMD
		...
		final Unit one = new Unit(new Player(1, "", FAKE_FILENAME), "unitType", "unitName", 1, FAKE_FILENAME);
		final String oldKindProperty = "type"; // NOPMD
		assertDeprecatedDeserialization(
				"Deserialize properly with deprecated use of 'type' for unit kind",
				one, createSerializedForm(one, true).replace(KIND_PROPERTY, oldKindProperty),
						Unit.class, oldKindProperty);
		assertDeprecatedDeserialization(
				"Deserialize properly with deprecated use of 'type' for unit kind",
				one, createSerializedForm(one, false).replace(KIND_PROPERTY, oldKindProperty),
						Unit.class, oldKindProperty);
		...
		assertSerialization(
				"Deserialize unit with no kind properly, reflection", new Unit(
						new Player(2, "", FAKE_FILENAME), "", NAME_PROPERTY, 2, FAKE_FILENAME), Unit.class,
				new Warning(Warning.Action.Ignore));
		assertMissingPropertyDeserialization(
				"Deserialize unit with no owner properly", new Unit(new Player(
						-1, "", FAKE_FILENAME), "kind", "unitThree", 3, FAKE_FILENAME),
				"<unit kind=\"kind\" name=\"unitThree\" id=\"3\" />",
				Unit.class, "owner");
		final Unit four = new Unit(new Player(3, "", FAKE_FILENAME), "unitKind", "", 4, FAKE_FILENAME);
		assertMissingPropertyDeserialization(
				"Deserialize unit with no name properly", four,
				createSerializedForm(four, true), Unit.class, NAME_PROPERTY);
		assertMissingPropertyDeserialization(
				"Deserialize unit with no name properly", four, createSerializedForm(four, false),
